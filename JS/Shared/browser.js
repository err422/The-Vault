...
/**
 * Main entry point - opens an iframe in the browser
...
function openIframe(url, title) {
    ...
    // Remove star manager if it exists
...
/**
 * Creates the browser window overlay
 */
function createBrowserWindow() {
    ...
    setTimeout(() => {
        // Check if bell schedule function exists
        if (typeof addBellScheduleIconToToolbar === 'function') {
            addBellScheduleIconToToolbar();
        } else if (typeof addBellIconToToolbar === 'function') {
            addBellIconToToolbar();
        }
        
        // Add favorites icon
        if (typeof addFavoritesIconToToolbar === 'function') {
...
/**
 * Creates a control button for the window
 */
...
/**
 * Shows the selection screen for choosing a new tab
 */
function showSelectionScreen() {
    ...
    // Gather all available items (games and websites)
...
function createNewTab(url, title) {
    ...
    // Get icon from allGames or allWebsites
    let icon = '🌐';
    if (typeof allGames !== 'undefined' && Array.isArray(allGames)) {
    ...
    // Start playtime tracking when tab is created
...
function switchToTab(tabId) {
    ...
    // Switch playtime tracking to this tab
...
function closeTab(tabId) {
    // Stop playtime tracking when tab closes
    const tabData = browserTabs.find(t => t.id === tabId);
    if (tabData && tabData.id === `tab-${tabCounter}`) {
        if (typeof stopPlaytimeTracking === 'function') {
...
/**
 * Creates navigation buttons (back, forward, reload)
 */
