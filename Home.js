...
// Load both games and websites data
...
searchInput.addEventListener('input', function() {
    ...
    const matchedWebsites = allWebsites.filter(website => 
...
function displayResults(results) {
    ...
    if (results.length === 0) {
        searchResults.innerHTML = '<p style="text-align: center; color: #888; padding: 40px;">No results found</p>';
        return;
    }
    ...
    results.forEach(item => {
        ...
        // Determine if it's a game or website
        const isGame = allGames.some(g => g.title === item.title);
        const category = item.category || (isGame ? 'Game' : 'Website');
        ...
        card.innerHTML = `
            ...
            <div class="game-meta">
                <span class="game-category">${category.toUpperCase()}</span>
        ...
        card.addEventListener('click', () => {
            const url = item.url || './error.html';
            openInBrowser(url, item.title);
        });
        
        searchResults.appendChild(card);
    });
    
    // Add stars to the search results
...
function renderFavorites() {
    ...
    favoriteItems.forEach(item => {
        ...
        item.addEventListener('click', function(e) {
            ...
            if (favorite && !favorite.url) {
                const found = allGames.find(g => g.title === title) || allWebsites.find(w => w.title === title);
                if (found) {
            ...
            if (favorite && favorite.url) {
                ...
                openInBrowser(favorite.url, favorite.title);
            } else {
    ...
    removeButtons.forEach(btn => {
        ...
        btn.addEventListener('click', function(e) {
            ...
            renderFavorites();
            updateSearchResultStars();
        });
    });
}

function addStarsToSearchResults() {
    const cards = document.querySelectorAll('#search-results .game-card');
    ...
    cards.forEach(card => {
        ...
        star.addEventListener('click', function(e) {
            ...
            let itemData = allGames.find(g => g.title === title) || allWebsites.find(w => w.title === title);
...
function updateSearchResultStars() {
    const stars = document.querySelectorAll('#search-results .favorite-star');
...
function addBellScheduleIconToToolbar() {
    ...
    bellButton.addEventListener('click', function(e) {
        e.stopPropagation();
        if (typeof toggleBellSchedulePopup === 'function') {
            toggleBellSchedulePopup();
        } else {
            console.error('Bell schedule function not found');
        }
    });
